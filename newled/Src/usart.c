...
//串口结构体
Uart_Structure_t Uart_1; 
Uart_Structure_t Uart_2;
Uart_Structure_t Uart_3;
/* 互斥信号量句柄 */
...
int _write(int fd, char *pBuffer, int size)
{
	...
	HAL_UART_Transmit(&HMI_UART.huart, pBuffer, size, 0xff);
...
void USART_Init(UART_HandleTypeDef *huart, USART_TypeDef *uart, uint32_t baud)
{
	huart->Instance = uart;
...
void convert_t(unsigned int ch)
{
	uint8_t dat[15] = "0123456789ABCDEF";
	char pstr[5] = "12345";
	int ww, qw, bw, sw, gw;
	int hw2, lw2, hw1, lw1;
	...
	/*if (ww = ch / 10000)
		...
		*pstr++ = number2char(0);*/
	/*pstr[0] = number2char(ch / 10000);
	pstr[1] = number2char(ch % 10000 / 1000);
	pstr[2] = number2char(ch % 1000 / 100);
	pstr[3] = number2char(ch % 100 / 10);
	pstr[4] = number2char(ch % 10);*/
	/*pstr[0] = dat[ch / 10000];
	pstr[1] = dat[ch % 10000 / 1000];
	pstr[2] = dat[ch % 1000 / 100];
	pstr[3] = dat[ch % 100 / 10];
	pstr[4] = dat[ch % 10];*/
		//MY_USART_chars(&DEBUG_UART.huart, pstr);
		//MY_USART_SendData(&DEBUG_UART.huart, pstr,5);
}
/*
