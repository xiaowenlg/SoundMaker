...
void write_register_80_1byte(uint8_t address, uint8_t data) 
{
	...
	array[4] = address;
	array[5] = data;
	MY_USART_SendData(&Uart_2.huart, array, 6);
}
...
void write_multiple_register_80(uint8_t address, uint8_t data_length, uint8_t *data)
{
	...
	MY_USART_SendData(&Uart_2.huart, array, nDataLen);
}
//************************************
...
void write_variable_store_82_1word(uint16_t address, uint16_t data)
{
	...
	MY_USART_SendData(&Uart_2.huart, array, 8);
}
...
{
	while (HAL_GPIO_ReadPin(WTN_GPIO_Port, WTN_BUSY_Pin) != 1);
	Line_2A_WTN6(val);
